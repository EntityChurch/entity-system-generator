...
fn cast(store: &Store, value: Value, to: &str) -> Vec<u8> {
    ...
    put(
        ...
        entity(
            NUMERIC_CAST,
            vec![("value", bytes(&v)), ("to_type", text(to))],
        ),
    )
}
...
fn literal_evaluates_to_its_value() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &entity(LITERAL, vec![("value", int(7))]))),
        int(7)
    );
}
...
fn a_literal_holding_a_stored_null_round_trips() {
    ...
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &entity(LITERAL, vec![("value", Value::Null)]))),
        Value::Null
    );
}
...
fn a_literal_with_no_value_key_is_invalid_expression() {
    let s = Store::new();
    assert_eq!(
        code_of(&run(&s, &entity(LITERAL, vec![]))),
...
fn arithmetic_reads_operands_through_hash_references() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &arith(&s, "add", int(2), int(3)))),
        int(5)
    );
...
fn div_exact_stays_integer_inexact_promotes_to_float() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &arith(&s, "div", int(10), int(2)))),
        int(5)
    );
    assert_eq!(
        float_of(value_of(&run(&s, &arith(&s, "div", int(7), int(2))))),
        3.5
    );
}
...
fn inexact_div_converts_each_operand_before_dividing() {
    ...
    assert_ne!(
        got, 384_307_168_202_282_368.0,
        "that is the correctly-rounded quotient §4.1 does not use"
    );
}
...
fn integer_div_by_zero_errors_float_div_by_zero_is_ieee() {
    let s = Store::new();
    assert_eq!(
        code_of(&run(&s, &arith(&s, "div", int(1), int(0)))),
        CODE_DIVISION_BY_ZERO
    );
    assert_eq!(
        code_of(&run(&s, &arith(&s, "mod", int(1), int(0)))),
        CODE_DIVISION_BY_ZERO
    );
    assert_eq!(
        float_of(value_of(&run(
            &s,
            &arith(&s, "div", Value::Float(1.0), int(0))
        ))),
        f64::INFINITY
    );
    let neg = float_of(value_of(&run(
        &s,
        &arith(&s, "div", Value::Float(1.0), Value::Float(-0.0)),
    )));
    assert_eq!(neg, f64::NEG_INFINITY, "IEEE distinguishes -0.0");
    assert!(float_of(value_of(&run(
        &s,
        &arith(&s, "div", Value::Float(0.0), int(0))
    )))
    .is_nan());
    ...
    assert_eq!(
        code_of(&run(
            &s,
            &arith(&s, "mod", Value::Float(1.0), Value::Float(0.0))
        )),
        CODE_DIVISION_BY_ZERO
    );
}
...
fn mod_is_truncated_not_floored() {
    ...
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mod", int(-7), int(2)))),
        int(-1)
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mod", int(7), int(-2)))),
    ...
    assert_eq!(
        float_of(value_of(&run(
            &s,
            &arith(&s, "mod", Value::Float(-7.5), int(2))
        ))),
        -1.5
    );
}
...
fn add_sub_mul_are_sign_agnostic_and_wrap_at_2_64() {
    ...
    assert_eq!(
        value_of(&run(&s, &arith(&s, "add", int(max_u), int(1)))),
        int(0)
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "sub", int(i64::MIN as i128), int(1)))),
        int(i64::MAX as i128)
    );
}
...
fn mul_wraps_past_i128_and_keeps_the_sign_of_the_true_product() {
    ...
    let max_u = u64::MAX as i128;
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mul", int(max_u), int(max_u)))),
        int(1),
        "(2^64-1)^2 mod 2^64"
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mul", int(1 << 63), int(2)))),
        int(0)
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mul", int(-1), int(3)))),
        int(-3)
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mul", int(-2), int(-3)))),
        int(6)
    );
    assert_eq!(
        value_of(&run(&s, &arith(&s, "mul", int(0), int(-3)))),
        int(0),
...
fn div_is_signed_default_unless_a_direct_uint_cast_is_the_operand() {
    ...
    assert_eq!(
        value_of(&run(&s, &arith(&s, "div", int(big), int(2)))),
        int(-1)
    );
    ...
    assert_eq!(
        value_of(&run(&s, &direct)),
        int(((u64::MAX - 1) / 2) as i128)
    );
...
fn eq_across_type_classes_is_false_not_a_type_mismatch() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &compare(&s, "eq", int(1), text("1")))),
        Value::Bool(false)
    );
    assert_eq!(
        value_of(&run(&s, &compare(&s, "neq", int(1), text("1")))),
        Value::Bool(true)
    );
    ...
    assert_eq!(
        value_of(&run(&s, &compare(&s, "eq", Value::Bool(true), int(1)))),
        Value::Bool(false)
    );
}
...
fn eq_over_ints_and_floats_is_one_type_class() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &compare(&s, "eq", int(1), Value::Float(1.0)))),
        Value::Bool(true)
    );
    ...
    assert_eq!(
        value_of(&run(&s, &compare(&s, "eq", a.clone(), b))),
        Value::Bool(false)
    );
    assert_eq!(
        value_of(&run(&s, &compare(&s, "eq", a.clone(), a))),
        Value::Bool(true)
    );
}
...
fn ordering_across_type_classes_is_a_type_mismatch_and_strings_order_by_utf8_bytes() {
    let s = Store::new();
    assert_eq!(
        code_of(&run(&s, &compare(&s, "lt", int(1), text("a")))),
        CODE_TYPE_MISMATCH
    );
    // "Z" (0x5A) < "a" (0x61) by bytes; "é" (0xC3 0xA9) > "z" by bytes.
    assert_eq!(
        value_of(&run(&s, &compare(&s, "lt", text("Z"), text("a")))),
        Value::Bool(true)
    );
    assert_eq!(
        value_of(&run(&s, &compare(&s, "gt", text("é"), text("z")))),
        Value::Bool(true)
    );
    ...
    assert_eq!(
        value_of(&run(
            &s,
            &compare(&s, "lte", Value::Float(f64::NAN), int(1))
        )),
        Value::Bool(false)
    );
    assert_eq!(
        value_of(&run(
            &s,
            &compare(&s, "gte", Value::Float(f64::NAN), int(1))
        )),
        Value::Bool(false)
    );
}
...
fn truthiness_ladder() {
    ...
    for (v, expected) in falsy
        .iter()
        .map(|v| (v, false))
        .chain(truthy.iter().map(|v| (v, true)))
    {
...
fn a_falsy_if_with_no_else_returns_null_not_an_error() {
    ...
    assert_eq!(
        value_of(&run(
            &s,
            &entity(IF, vec![("condition", bytes(&c)), ("then", bytes(&one))])
        )),
        Value::Null
    );
}
...
fn a_missing_scope_name_is_not_found_and_a_null_binding_is_still_a_binding() {
    ...
    assert_eq!(
        code_of(&run(
            &s,
            &entity(LOOKUP_SCOPE, vec![("name", text("nope"))])
        )),
        CODE_NOT_FOUND
    );
...
fn lookup_tree_evaluates_a_stored_expression_and_returns_a_stored_value() {
    ...
    assert_eq!(
        value_of(&run(
            &s,
            &entity(LOOKUP_TREE, vec![("path", text("app/sum"))])
        )),
        int(2)
    );
    assert_eq!(
        entity_of(&run(
            &s,
            &entity(LOOKUP_TREE, vec![("path", text("app/thing"))])
        ))
        .hash,
        value.hash
    );
...
fn lookup_tree_canonicalizes_and_the_dependency_records_the_canonical_form() {
    ...
    let out = ev.evaluate_at(
        &entity(LOOKUP_TREE, vec![("path", text("app/v"))]),
        &root(),
...
fn a_reserved_relative_form_canonicalizes_to_the_sentinel_and_is_not_a_dependency() {
    ...
    assert!(
        ev.dependencies().is_empty(),
        "the sentinel is not a path: {:?}",
        ev.dependencies()
    );
    ...
    let _ = ev2.evaluate_at(
        &entity(LOOKUP_TREE, vec![("path", text("app/x"))]),
        &root(),
        EvaluateOptions::default(),
    );
...
fn a_reserved_relative_form_fails_closed_under_a_real_grant() {
    ...
    let out = ev.evaluate_at(
        ...
        &root(),
        EvaluateOptions {
            can_read_path: Some(&deny_sentinel),
            ..Default::default()
        },
    );
...
fn relative_lookup_resolves_against_the_subgraph_root() {
    let s = Store::new();
    s.bind(
        &format!("/{PEER}/app/sibling"),
        &entity("app/v", vec![("n", int(3))]),
    );
    let e = entity(
        LOOKUP_TREE,
        vec![
...
fn a_read_predicate_that_says_no_denies_before_the_dependency_is_registered() {
    ...
    assert!(
        ev.dependencies().is_empty(),
        "an unauthorized path must not leak through a registered dependency"
    );
}
...
fn index_length_and_numeric_cast_evaluate_and_are_in_both_membership_lists() {
    ...
    assert_eq!(
        value_of(&run(
            &s,
            &entity(INDEX, vec![("array", bytes(&arr)), ("index", bytes(&one))])
        )),
        int(20)
    );
    assert_eq!(
        value_of(&run(&s, &entity(LENGTH, vec![("array", bytes(&arr))]))),
        int(2)
    );
    ...
    assert_eq!(
        code_of(&run(
            &s,
            &entity(INDEX, vec![("array", bytes(&arr)), ("index", bytes(&neg))])
        )),
        CODE_INDEX_OUT_OF_RANGE
    );
    let hello = literal(&s, text("hello"));
    assert_eq!(
        code_of(&run(&s, &entity(LENGTH, vec![("array", bytes(&hello))]))),
        CODE_TYPE_MISMATCH
    );
}
...
fn casting_nan_or_inf_to_an_integer_is_cast_out_of_range_and_negative_int_to_uint_reinterprets() {
    ...
    for v in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        ...
        assert_eq!(
            code_of(&run(&s, &s.get_by_hash(&c).unwrap())),
            CODE_CAST_OUT_OF_RANGE
        );
    }
    let c = cast(&s, int(-1), "primitive/uint");
    assert_eq!(
        value_of(&run(&s, &s.get_by_hash(&c).unwrap())),
        int(u64::MAX as i128)
    );
...
fn float_casts_range_check_before_any_native_cast() {
    ...
    assert_eq!(
        code_of(&run_cast(two63, "primitive/int")),
        CODE_CAST_OUT_OF_RANGE
    );
    assert_eq!(
        value_of(&run_cast(-two63, "primitive/int")),
        int(i64::MIN as i128)
    );
    assert_eq!(
        code_of(&run_cast(two64, "primitive/uint")),
        CODE_CAST_OUT_OF_RANGE
    );
    assert_eq!(
        value_of(&run_cast(1e19, "primitive/uint")),
        int(10_000_000_000_000_000_000)
    );
    assert_eq!(
        value_of(&run_cast(-0.5, "primitive/uint")),
        int(0),
        "trunc(-0.5) is -0.0, which is not < 0"
    );
...
fn the_budget_charges_evaluate_steps_and_nothing_else() {
    ...
    assert_eq!(
        run(&s, &arith(&s, "add", int(1), int(2))).operations_used,
        3
    );
}
...
fn an_exhausted_budget_yields_budget_exhausted_and_a_caller_cannot_raise_its_ceiling() {
    ...
    let tight = EvaluatorLimits {
        max_operations: 2,
        max_depth: 64,
    };
    assert_eq!(
        code_of(&run_with(&s, &e, tight, EvaluateOptions::default())),
        CODE_BUDGET_EXHAUSTED
    );
    let asked = EvaluateOptions {
        budget: Some(1_000_000),
        ..Default::default()
    };
    assert_eq!(
        code_of(&run_with(&s, &e, tight, asked)),
        CODE_BUDGET_EXHAUSTED,
        "§5.2's minimum rule"
    );
}
...
fn a_stored_compute_error_short_circuits_its_consumer_and_materializes_code_only() {
    ...
    let e = entity(
        IF,
        vec![("condition", bytes(&stored)), ("then", bytes(&one))],
    );
    let out = run(&s, &e);
    ...
    assert_eq!(
        materialized,
        entity(ERROR, vec![("code", text("custom_code"))]),
        "message is never materialized"
    );
}
...
fn identity_closure_apply(s: &Store, arg: Value) -> Entity {
    ...
    entity(
        APPLY,
        vec![("fn", bytes(&lam)), ("args", map(vec![("x", bytes(&a))]))],
    )
}
...
fn a_lambda_applies_and_a_missing_argument_is_named_after_the_param() {
    let s = Store::new();
    assert_eq!(
        value_of(&run(&s, &identity_closure_apply(&s, int(5)))),
        int(5)
    );
    let x = put(&s, entity(LOOKUP_SCOPE, vec![("name", text("x"))]));
    let lam = put(
...
fn a_closure_captures_its_scope() {
    ...
    let lam = put(
        ...
        entity(
            LAMBDA,
            vec![("params", Value::Array(vec![])), ("body", bytes(&y))],
        ),
    );
...
fn a_stored_closure_applied_reaches_load_scope() {
    ...
    let dangling_binding = map(vec![
        ("kind", text("entity")),
        ("entity_hash", bytes(&[9; 33])),
    ]);
    let env = put(
        &s,
        entity(
            SCOPE,
            vec![("bindings", map(vec![("gone", dangling_binding)]))],
        ),
    );
    let closure = put(
        &s,
    ...
    assert_eq!(
        code_of(&run(&s, &entity(APPLY, vec![("fn", bytes(&closure))]))),
...
fn each_value_type_evaluates_to_itself_and_an_unknown_type_is_still_unknown_type() {
    ...
    for e in [
        entity(
            CLOSURE,
            vec![("params", Value::Array(vec![])), ("body", bytes(&body))],
        ),
        entity(SCOPE, vec![("bindings", map(vec![]))]),
        entity(
            RESULT,
            vec![("value", int(1)), ("expression", bytes(&body))],
        ),
    ] {
    ...
    assert_eq!(
        code_of(&run(&s, &entity("compute/nonsense", vec![]))),
        CODE_UNKNOWN_TYPE
    );
    assert_eq!(
        code_of(&run(&s, &entity("app/thing", vec![]))),
        CODE_UNKNOWN_TYPE
    );
}
...
fn builtin(_s: &Store, name: &str, args: Vec<(&str, Vec<u8>)>) -> Entity {
    entity(
        ...
        vec![
            ...
            (
                "args",
                map(args.into_iter().map(|(k, v)| (k, bytes(&v))).collect()),
            ),
        ],
    )
}
    ...
    put(
        ...
        entity(
            ...
            vec![
                (
                    "params",
                    Value::Array(params.iter().map(|p| text(p)).collect()),
                ),
                ("body", bytes(&body)),
            ],
        ),
...
fn a_builtin_path_apply_carrying_capability_or_resource_is_invalid_and_a_near_miss_is_not_a_builtin(
    ...
    let bad = entity(
        APPLY,
        vec![
            ("path", text(&format!("{BUILTINS_PREFIX}/map"))),
            ("operation", text("eval")),
            ("resource", bytes(&h)),
        ],
    );
    ...
    let near = entity(
        APPLY,
        vec![
            ("path", text("system/compute/builtinsmap")),
            ("operation", text("eval")),
        ],
    );
    let out = run(&s, &near);
    ...
    assert!(
        ...
        "a near miss is handler mode, not a builtin"
    );
}
...
fn map_contains_a_failing_element_and_filter_short_circuits() {
    ...
    let mapped = value_of(&run(
        &s,
        &builtin(
            &s,
            "map",
            vec![("collection", coll.clone()), ("fn", f.clone())],
        ),
    ));
    let Value::Array(items) = mapped else {
        panic!()
    };
    ...
    assert_eq!(
        items[1],
        Value::Bytes(err_hash),
        "a contained error is a code-only entity, by hash"
    );

    assert_eq!(
        code_of(&run(
            &s,
            &builtin(&s, "filter", vec![("collection", coll), ("fn", f)])
        )),
        CODE_DIVISION_BY_ZERO
    );
}
    ...
    assert_eq!(
        value_of(&run(
            ...
            &builtin(
                &s,
                "fold",
                vec![("collection", empty), ("fn", f), ("initial", init)]
            )
        )),
        text("start")
    );
...
fn range_refuses_rather_than_clamping() {
    ...
    assert_eq!(
        value_of(&run(&s, &builtin(&s, "range", vec![("n", three)]))),
        Value::Array(vec![int(0), int(1), int(2)])
    );
    let neg = literal(&s, int(-1));
    assert_eq!(
        code_of(&run(&s, &builtin(&s, "range", vec![("n", neg)]))),
    ...
    assert_eq!(
        code_of(&run(&s, &builtin(&s, "range", vec![("n", huge)]))),
        CODE_COUNT_OUT_OF_RANGE
    );
}
...
fn concat_joins_one_level_and_assoc_uses_index_codes() {
    ...
    let nested = literal(
        ...
        Value::Array(vec![
            ...
            Value::Array(vec![Value::Array(vec![int(2)])]),
        ]),
    );
    assert_eq!(
        value_of(&run(
            &s,
            &builtin(&s, "concat", vec![("collections", nested)])
        )),
    ...
    assert_eq!(
        value_of(&run(
            ...
            &builtin(
                &s,
                "assoc",
                vec![("collection", coll), ("index", idx0), ("value", v)]
            )
        )),
        Value::Array(vec![int(9), int(2)])
    );
}
...
fn group_by_orders_groups_by_first_appearance_of_their_key() {
    ...
    let coll = literal(
        &s,
        Value::Array(vec![int(3), int(1), int(4), int(1), int(5)]),
    );
    let x = put(&s, entity(LOOKUP_SCOPE, vec![("name", text("x"))]));
    let two = literal(&s, int(2));
    let parity = put(
    ...
    let Value::Array(groups) = value_of(&run(
        &s,
        &builtin(&s, "group-by", vec![("collection", coll), ("fn", f)]),
    )) else {
        panic!()
    };
    ...
    assert_eq!(
        first.field("key"),
        Some(&int(1)),
        "3 appears first, and 3 mod 2 is 1"
    );
    assert_eq!(
        first.field("members"),
        Some(&Value::Array(vec![int(3), int(1), int(1), int(5)]))
    );
}
...
fn the_construct_alias_and_the_inline_form_agree_on_the_materialized_hash() {
    ...
    let inline = entity(
        CONSTRUCT,
        vec![
            ("entity_type", text("app/pair")),
            ("fields", map(vec![("a", bytes(&a)), ("b", bytes(&b))])),
        ],
    );
...
fn construct_fields_evaluate_in_ecf_canonical_key_order() {
    ...
    let e = entity(
        CONSTRUCT,
        vec![
            ("entity_type", text("app/t")),
            (
                "fields",
                map(vec![("aa", bytes(&aa_div)), ("z", bytes(&z_mod))]),
            ),
        ],
    );
    let out = run(&s, &e);
    ...
    assert!(
        out.error.unwrap().detail.contains("Modulo"),
        "`z` (mod) is evaluated before `aa` (div)"
    );
}
...
fn navigation_composes_through_a_construct_inside_one_evaluation_and_a_readback_returns_the_hash() {
    ...
    let inner = put(
        ...
        entity(
            CONSTRUCT,
            vec![
                ("entity_type", text("app/inner")),
                ("fields", map(vec![("name", bytes(&n))])),
            ],
        ),
    );
    let outer = put(
        ...
        entity(
            CONSTRUCT,
            vec![
                ("entity_type", text("app/outer")),
                ("fields", map(vec![("inner", bytes(&inner))])),
            ],
        ),
    );
    let f1 = put(
        ...
        entity(
            FIELD,
            vec![("name", text("inner")), ("entity", bytes(&outer))],
        ),
    );
    ...
    let readback = entity(
        FIELD,
        vec![("name", text("inner")), ("entity", bytes(&look))],
    );
...
fn field_navigation_requires_an_entity_or_record() {
    ...
    assert_eq!(
        code_of(&run(
            &s,
            &entity(FIELD, vec![("name", text("x")), ("entity", bytes(&five))])
        )),
        CODE_TYPE_MISMATCH
    );
}
...
fn resolve_rejects_a_non_compute_entity_and_tiers_0_and_2_and_included_admit_it() {
    ...
    assert_eq!(
        code_of(&run(&s, &e)),
        CODE_NOT_FOUND,
        "compute is not a content-store oracle"
    );
    ...
    let included: BTreeMap<Vec<u8>, Entity> =
        [(only_included.hash.clone(), only_included.clone())].into();
