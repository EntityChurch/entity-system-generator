...
fn thirty_three_types_are_defined_once_each_in_all_types_order() {
    ...
    assert_eq!(defs.iter().map(|(n, _)| *n).collect::<Vec<_>>(), ALL_TYPES.to_vec());
    let hashes: HashSet<_> = compute_type_entities().into_iter().map(|(_, e)| e.hash).collect();
    assert_eq!(hashes.len(), 33, "no two definitions render the same entity");
...
fn fields_of(name: &str) -> Vec<String> {
    let (_, data) = compute_type_defs().into_iter().find(|(n, _)| *n == name).unwrap();
    ...
    match fields {
        Some(Value::Map(f)) => f.into_iter().map(|(k, _)| match k { Key::Text(t) => t, _ => panic!() }).collect(),
        _ => vec![],
    }
}
...
fn the_membership_predicates_carry_the_three_types_sections_4_2_and_4_7_omit() {
    let expressions: Vec<_> = ALL_TYPES.iter().filter(|t| is_compute_expression(t)).collect();
    ...
    for value_type in [CLOSURE, SCOPE, RESULT, ERROR] {
        assert!(is_compute_type(value_type) && !is_compute_expression(value_type), "{value_type}");
    }
    assert!(!is_compute_type(SCOPE_BINDING) && !is_compute_type(GROUP), "record types are neither");
}
...
fn an_optional_field_spec_is_the_inner_spec_plus_optional_true() {
    let (_, data) = compute_type_defs().into_iter().find(|(n, _)| *n == LOGIC).unwrap();
    let expected = map(vec![
        ("name", text(LOGIC)),
        ("fields", map(vec![
            ("op", map(vec![("type_ref", text("primitive/string"))])),
            ("left", map(vec![("type_ref", text("system/hash"))])),
            ("right", map(vec![("type_ref", text("system/hash")), ("optional", Value::Bool(true))])),
        ])),
    ]);
    assert_eq!(
