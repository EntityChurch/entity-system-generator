...
fn the_public_routes_have_the_signatures_the_composition_calls() {
    let _: fn(&Arc<Peer>, EvaluatorLimits) -> Result<ComputeInstallation, RegisterError> =
        install_compute;
...
fn install_compute_installs_every_face_and_reads_the_evaluator_back() {
    let p = peer();
    assert!(
        !p.has_native_handler(COMPUTE_PATTERN),
        "control: a fresh peer has no body at the pattern"
    );
    assert!(
        p.expression_evaluator().is_none(),
        "control: a fresh peer has no evaluator"
    );
    ...
    assert_eq!(
        p.store.get_at(&abs(&p, COMPUTE_PATTERN)).map(|e| e.typ),
        Some("system/handler".to_string())
    );
    assert_eq!(
        installed.interface_path,
        abs(&p, &format!("system/handler/{COMPUTE_PATTERN}"))
    );
    ...
    assert!(p
        ...
        .get_at(&abs(
            &p,
            &format!("system/capability/grants/{COMPUTE_PATTERN}")
        ))
        .is_some());
...
fn a_second_install_is_refused_and_leaves_the_first_in_place() {
    ...
    let first = p
        ...
        .expect("evaluator after the first install");
...
fn the_override_guard_refuses_a_builtin_pattern_and_not_a_near_miss_or_our_own() {
    for bad in [
        BUILTINS_PREFIX.to_string(),
        format!("{BUILTINS_PREFIX}/map"),
        format!("/{PEER}/{BUILTINS_PREFIX}/add"),
    ] {
        assert!(assert_not_builtin_override(&bad).is_err(), "{bad}");
    }
    for fine in [
        COMPUTE_PATTERN,
        "system/compute/builtinsmap",
        "app/system/compute/builtins",
    ] {
...
fn install_compute_registers_the_consumer_and_rebuilds_from_the_tree() {
    ...
    let p2 = peer();
    p2.store
        .bind(&abs(&p2, "app/A1"), &entity("app/cell", vec![]));
    let root = abs(&p2, "app/B1");
    p2.store
        .bind(&root, &entity(LOOKUP_TREE, vec![("path", text("app/A1"))]));
    p2.store.bind(
        &format!("{}/{}", abs(&p2, PROCESSES_PREFIX), deterministic_id(&root)),
        &entity(
            SUBGRAPH,
            vec![
                ("root_expression_path", text(&root)),
                ("status", text("active")),
                ("result_path", text(&format!("{root}/result"))),
            ],
        ),
    );
