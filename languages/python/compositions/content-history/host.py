...
def main(argv: list[str]) -> int:
    ...
    def configure(peer) -> None:
        ...
        #
        # `sdk-native` for both: this peer refuses a wire register at a `system/*` pattern
        # and both patterns are `system/*`. Not a shortcut -- the only route.
        #
        ...
        #
        # Not extension code. §6.1: configuration "uses the standard tree `put`" and needs
        # no handler operation, so which paths a deployment audits is the composition's
        # call.
        #
        ...
        sys.stderr.write(
            ...
            # `no` by what the recorder actually saw. The counts are printed beside it
            # because a verdict with no quantity under it cannot be debugged by the person
            # who runs it.
