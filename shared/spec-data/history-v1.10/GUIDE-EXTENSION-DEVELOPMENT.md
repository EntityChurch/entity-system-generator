...
**`ENTITY-CORE-PROTOCOL.md` §3.3's 400 row is the authority for that status**, which enumerates `path_required` by name alongside `invalid_path`, `invalid_params`, `unexpected_params`, `chain_depth_exceeded` and `signature_path_conflict`. It is **not** a forbidden synonym for `invalid_request`: a synonym is a differently-spelled generic, whereas `path_required` names a specific remediable defect, and the code selects the remedy — *supply a resource* is a different instruction from *fix your request*. Because the condition is raised at dispatch, before any handler runs, the code is available to **every** handler and no extension declares it in its own code set.

> **This paragraph claimed to be the authority for the status until 2026-09-08, and it was not.** A guide teaches; it does not own a wire-observable status code. The claim rested on the core row *"sanctioning them by name"* — but that row named five spellings, an enumeration and not a category, and `path_required` was not among them. So an extension `MUST`-ing the code was emitting a spelling no code set defined, which the extension's own appendix rule calls non-conformant, while the conformance suite asserted the literal string. **The core row now names it**, and this section restates rather than owns it. **The shape is worth recognising elsewhere: a rule stated in a non-normative document acquires normative weight from being cited, and then the citations are what keep it alive.**
