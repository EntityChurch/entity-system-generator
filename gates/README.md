...
| **`chunking-parity`** | `EXTENSION-CONTENT` §3.7 — §3.2/§3.6 are **Conformance** algorithms. **Nothing upstream measures it**: no oracle check chunks anything (verified against the oracle's executed `content` check set, 2026-09-06), and §3.6.5's vectors do not exist yet | one corpus → `languages/<t>/gates/chunking-parity/run` → the neutral `gates/chunking-parity/compare.py` | `make parity`, a `wildcard` over every arm; the count is echoed before the run | **built 2026-09-06.** 3 ports agree on all 6 fields |
...
| **`eligible`** | **keystone's `KEYSTONE-PEER-REPORT.json`** — keystone certifies its own peer against the keystone peer contract; we are one consumer and never re-measure what the report certifies | `tools/check-eligible.py` — each extension's `EXTENSION.toml [requires]` plus `gates/eligible/LAUNCH.toml`, against the report's rows, keystone's vocabulary, the recomputed `contract_digest` and the certified commit | `make eligible`, **first in `make check`**; `make eligible-control` runs 19 planted cases and 4 refusals | **built 2026-09-13.** `rust`: all three compositions ELIGIBLE, `embed.data` PENDING on every extension. `typescript`/`python`: declared PRE-CONTRACT (K-18). A planted `fail` row on keystone's real report turns it red; a declined MODULE row reddens `compute` and leaves `content` green |
...
simply stop deduplicating, with no error and no status code. Checked against the oracle's own
executed check set (2026-09-06): not one of
